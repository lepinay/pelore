class CRTEffect {
    ...
    applyCanvas(time) {
        ...
        if (!vignetteCtx)
            return;
        const gradient = vignetteCtx.createRadialGradient(width / 2, height / 2, 0, width / 2, height / 2, Math.max(width, height) / 1.5);
        ...
        if (Math.random() < noiseThreshold) {
            ...
            for (let i = 0; i < noiseCount; i++) {
                this.ctx.fillRect(Math.random() * width, Math.random() * height, Math.random() * 3 + (this.params.noise * 5), Math.random() * 3 + (this.params.noise * 5));
            }
        }
    }
    ...
    detectBeat(audioData, startBin, endBin, threshold) {
        let sum = 0;
...
//# sourceMappingURL=CRTEffect.js.map